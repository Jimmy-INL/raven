...
"""
  ...
  @revison log: khnguy22 make modification to accomodate EQ optimization problem
"""
# External Modules----------------------------------------------------------------------------------
...
class EQMonteCarlo(Sampler):
  """
    EQ MONTE CARLO Sampler
  """
  ...
  def getInputSpecification(cls):
    ...
    """
    inputSpecification = super(EQMonteCarlo, cls).getInputSpecification()
    ...
    ## add coreinput and xmlinput
    xmlinput = InputData.parameterInputFactory('xmlinput', InputTypes.StringType, printPriority=109,
    descr=r""" Xml input for the decodeing of EQ shuffle scheme. """)
    # ## add type and class here
    # xmlinput.addParam("class", InputTypes.StringType, True)
    # xmlinput.addParam("type", InputTypes.StringType, True)
    samplerInitInput.addSub(xmlinput)

    coreinput = InputData.parameterInputFactory('coreinput', InputTypes.StringType, printPriority=109,
    descr=r""" Code input for the handling the symmertry of core. """)
    # coreinput.addParam("class", InputTypes.StringType, True)
    # coreinput.addParam("type", InputTypes.StringType, True)
    samplerInitInput.addSub(coreinput)
  ...
  def __init__(self):
    ...
    """
    super().__init__()
    self.printTag = 'CUSTOM SAMPLER MONTECARLO for EQ problem'
  ...
  def localInputAndChecks(self, xmlNode, paramInput):
    ...
    if paramInput.findFirst('samplerInit') is not None:
      if self.limit is None:
        self.raiseAnError(IOError,self, f'Monte Carlo sampler EQ {self.name} needs the limit block (number of samples) in the samplerInit block')
      ...
      self.raiseAnError(IOError, self, f'Monte Carlo sampler EQ {self.name} needs the samplerInit block')
    ## add core input and xml input
    coreinput = paramInput.findFirst('samplerInit').findFirst('coreinput')
    if coreinput is not None:
      self.coreinput = coreinput.value
    else:
      self.raiseAnError(IOError,'Please ensure that the coreinput block is specified for the this sampler')
    xmlinput = paramInput.findFirst('samplerInit').findFirst('xmlinput')
    if xmlinput is not None:
      self.xmlinput = xmlinput.value
    else:
      self.raiseAnError(IOError,'Please ensure that the xml block is specified for the this sampler')
  ...
  def localGenerateInput(self, model, myInput):
    """
      ...
      @ Out, None
    """
    # use class from utils rather than in Sampler itself for adaptablity later in cross-over and mutation
    EQcheck = EQchecker(self.xmlinput, self.coreinput)
    all_key = sorted(self.distDict)
    _, genome = EQcheck.create_genome()
    # EQcheck.checkgennome(genome)
    # assigning values and weight?
    weight = 1.0
    for i,key in enumerate(sorted(self.distDict)):
      ...
      weight = 1.0
      if totDim !=1:
        raise KeyError("This EQ sampler does not support multi-dimension sampling!")
      else:
        rvsnum = genome[i]
