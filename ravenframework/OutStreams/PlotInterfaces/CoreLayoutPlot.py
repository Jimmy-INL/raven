...
class CoreLayoutPlot(PlotInterface):
  ...
  def getInputSpecification(cls):
    ...
    spec.addSub(InputData.parameterInputFactory('label_precision', contentType=InputTypes.IntegerType,
        descr=r"""Number of decimal places for numeric cell labels. If omitted, it is chosen from
                  the data: 0 decimals for integer-valued fields (fuel ID, burn tier) and 2 for
                  fields with fractional values (enrichment, RPF, exposure), so a 2.5 wt% assembly
                  reads '2.50' rather than being rounded to '2'."""))
  ...
  def handleInput(self, spec):
    ...
    if lbl is not None and lbl.value:
      ...
      self.labelMode = mode
    prec = spec.findFirst('label_precision')
    if prec is not None and prec.value is not None:
      self.labelPrecision = max(0, int(prec.value))
  ...
  def initialize(self, stepEntities):
    ...
    if not self.locToCoords:
      ...
      # PRIORITY 2: If no explicit path, try to find it near the source (your current logic)
        ...
        import os
        import glob
        # Safely attempt to get the path from the source object
        src_path = src.getPath() if hasattr(src, 'getPath') else getattr(src, 'filename', None)

        if src_path is None:
          self.raiseAnError(IOError,
            f'CoreLayoutPlot "{self.name}" could not find locXX mappings in the template, '
            f'and the source "{self.sourceName}" is a data object with no file path to search. '
  ...
  def _emitPlotlySurface(self, grid_view, row_start, col_start, use_burn_tier):
    ...
    surface_kwargs = {}
    if use_burn_tier:
      tier_scale = self._burnTierScale(z_float)
      surface_kwargs.update(dict(colorscale=self._burnTierColorscale(tier_scale),
                                 zmin=-0.5, zmax=tier_scale['nTiers'] - 0.5, showscale=not self.categoricalLegend,
                                 colorbar=dict(title=dict(text=self.colorbarLabel or 'Burn tier',
                                                          font=dict(color='#e8f0ff')),
                                               tickvals=tier_scale['ticks'],
                                               ticktext=tier_scale['labels'],
  ...
  def _emitPlotlyBars(self, grid_view, row_start, col_start, use_burn_tier):
    ...
    colorbar_trace = None
    if use_burn_tier:
      tier_scale = self._burnTierScale(heights)
      tier_colors = tier_scale['colors']
      colorscale = self._burnTierColorscale(tier_scale)
      cmin, cmax = -0.5, tier_scale['nTiers'] - 0.5
      ...
      def color_for(val):
        if np.isnan(val):
          return '#444444'
        return tier_colors[min(max(int(round(val)), 0), len(tier_colors) - 1)]
  ...
  def _matplotlibBarFrame(self, grid_view, x_ticktext, y_ticktext, use_burn_tier, vmin=None, vmax=None, limit_plane=None):
    ...
    fig.patch.set_facecolor('#ffffff')  # light box on light figure -> all ticks readable

    if use_burn_tier:
      tier_scale = self._burnTierScale(heights)
      cmap = matplotlib.colors.ListedColormap(tier_scale['colors'])
      norm = matplotlib.colors.BoundaryNorm(tier_scale['bounds'], cmap.N)
    else:
    ...
    ax.set_xlabel('Column (A..)', color='#000000')
    ax.set_ylabel('Row (1..)', color='#000000')
    ax.set_zlabel(self.colorbarLabel or 'Value', color='#000000')
    ax.tick_params(colors='#000000')
    ax.set_xticks(range(len(x_ticktext)))
    ax.set_yticks(range(len(y_ticktext)))
    ax.set_xticklabels(x_ticktext, color='#000000')
    ax.set_yticklabels(y_ticktext, color='#000000')
    ax.view_init(elev=35, azim=-135)
    try:
      ax.set_box_aspect(None, zoom=1.15)  # enlarge the cube within the axes (mpl>=3.6)
    except TypeError:
      pass
  ...
  def _burnTierScale(self, values):
    """
      Build a data-driven discrete color scale for burn tier so the categories and labels
      match the tiers actually present in the data: 0=fresh, 1=once, 2=twice, 3=thrice,
      4+ = "N times". No "twice+" cap — the number of bands grows with the data. Colors
      run green (fresh) -> red (most burned) at any tier count.
      @ In, values, iterable of tier values (nan allowed)
      @ Out, scale, dict with keys colors, bounds, ticks, labels, nTiers, lightTiers
    """
    arr = np.asarray(list(values), dtype=float)
    finite = arr[np.isfinite(arr)]
    maxTier = int(round(float(np.max(finite)))) if finite.size else 2
    maxTier = max(maxTier, 0)
    nTiers = maxTier + 1
    names = ['fresh', 'once', 'twice', 'thrice']
    labels = [f'{t} {names[t] if t < len(names) else "times"}' for t in range(nTiers)]
    if nTiers == 1:
      colors = ['#2ca02c']
    else:
      base = plt.cm.get_cmap('RdYlGn_r')
      colors = [matplotlib.colors.rgb2hex(base(i / (nTiers - 1))) for i in range(nTiers)]
    # tiers whose fill is light enough to need dark label text (perceived luminance)
    lightTiers = set()
    for i, hexc in enumerate(colors):
      r, g, b = matplotlib.colors.to_rgb(hexc)
      if (0.299 * r + 0.587 * g + 0.114 * b) > 0.6:
        lightTiers.add(i)
    bounds = [t - 0.5 for t in range(nTiers + 1)]
    return {'colors': colors, 'bounds': bounds, 'ticks': list(range(nTiers)),
            'labels': labels, 'nTiers': nTiers, 'lightTiers': lightTiers}

  def _burnTierColorscale(self, scale):
    """
      Stepped plotly colorscale (normalized 0..1) from a burn-tier scale dict. Pair with
      cmin=-0.5, cmax=nTiers-0.5 and colorbar tickvals=scale['ticks'], ticktext=scale['labels'].
      @ In, scale, dict from _burnTierScale
      @ Out, cs, list of (position, color) tuples
    """
    colors = scale['colors']
    n = scale['nTiers']
    cs = []
  ...
  def run(self):
    ...
    if 'core_row' in df.columns and 'core_col' in df.columns:
      ...
      for _, row in df.iterrows():
        ...
        for loc, coords in self.locToCoords.items():
          if (r, c) in coords:
            active_vals[loc] = row[color_col]
            loc_found = True
            break
      if not active_vals:
        self.raiseAWarning(f'CoreLayoutPlot "{self.name}" found no matching locations in map for column "{color_col}".')
        return
    else:
      # Wide format: pick a single row and extract locXX columns
      ...
      if self.colorPrefix:
        ...
        # For burn_tier, also try reasonable fallbacks
      ...
      if self.colorPrefix and 'burn_tier' in self.colorPrefix.lower():
        for k, v in list(active_vals.items()):
          active_vals[k] = float(round(v)) if np.isfinite(v) else np.nan
      if not active_vals:
    ...
    use_burn_tier = bool(self.colorPrefix) and 'burn_tier' in self.colorPrefix.lower()
    stage_grids = []
    stage_labels = []
    ...
    if self.surface3d or self.bar3d:
      ...
      fig = plt.figure(figsize=(9.5, 8.0))
      ax = fig.add_subplot(111, projection='3d')
      # A light 3D box floating on a dark figure leaves the outer (z-axis) tick numbers
      # straddling the light/dark edge, half of them unreadable. Use a light figure so every
      # tick sits on a light background, and give the axes near-full-figure margins + a slight
      # zoom so the cube reads large.
    ...
    if self.surface3d or self.bar3d:
      ...
      if use_burn_tier:
        tier_scale = self._burnTierScale(vals)
        cmap = matplotlib.colors.ListedColormap(tier_scale['colors'])
        norm = matplotlib.colors.BoundaryNorm(tier_scale['bounds'], cmap.N)
      else:
        cmap = plt.cm.get_cmap(self.cmapName or 'viridis')
      ...
      # 3D box panes render light (matplotlib default), so tick labels/marks must be dark to read.
      ax.set_xticklabels([chr(ord('A') + col_start + i) for i in range(size_view)], color='#000000')
      ax.set_yticklabels([row_start + i + 1 for i in range(size_view)], color='#000000')
      ax.tick_params(colors='#000000')
      ax.set_title(f'Core layout: {self.name}', color='#000000')
      try:
        ax.set_box_aspect(None, zoom=1.15)  # enlarge the cube within the axes (mpl>=3.6)
      except TypeError:
        pass
      ...
      return
    if use_burn_tier:
      tier_scale = self._burnTierScale(grid_view)
      cmap = matplotlib.colors.ListedColormap(tier_scale['colors'])
      bounds = tier_scale['bounds']
      norm = matplotlib.colors.BoundaryNorm(bounds, cmap.N)
      img = ax.imshow(masked, cmap=cmap, norm=norm, origin='upper', interpolation='none')
      if not self.categoricalLegend:
        cbar = fig.colorbar(img, ax=ax, fraction=0.046, pad=0.04, ticks=tier_scale['ticks'], boundaries=bounds)
        ...
        cbar.ax.set_yticklabels(tier_scale['labels'])
        for lab in cbar.ax.get_yticklabels():
          lab.set_color('#e8f0ff')
      else:
        legend_handles = [Patch(facecolor=tier_scale['colors'][t], edgecolor='none', label=tier_scale['labels'][t])
                          for t in range(tier_scale['nTiers'])]
    ...
    # Decide numeric label precision: explicit override, else 0 decimals for integer-valued
    # fields (fuel ID, burn tier) and 2 for fractional fields (enrichment, RPF, exposure) so
    # a 2.5 wt% assembly reads '2.50' instead of collapsing to '2'.
    if self.labelPrecision is not None:
      labelPrec = self.labelPrecision
    else:
      finiteVals = [v for v in active_vals.values() if np.isfinite(v)]
      allInteger = bool(finiteVals) and all(float(v).is_integer() for v in finiteVals)
      labelPrec = 0 if allInteger else 2

    for loc, coords in locs_view.items():
      val = active_vals.get(loc, np.nan)
      display_loc = self.locLabels.get(loc, loc)
      # Choose text color to keep contrast on bright categories (e.g., yellow mid tiers).
      text_color = '#e8f0ff'
      if use_burn_tier and np.isfinite(val):
        if int(round(val)) in tier_scale['lightTiers']:
          text_color = '#000000'
      for r, c in coords:
        txt = ''
        if self.labelMode == 'value':
          txt = f'{val:.{labelPrec}f}' if np.isfinite(val) else ''
        elif self.labelMode == 'loc':
          txt = display_loc
        else:
          txt_val = f'{val:.{labelPrec}f}' if np.isfinite(val) else ''
    ...
    if self.bar3d:
      ...
      if use_burn_tier:
        tier_scale = self._burnTierScale(heights)
        cmap_bar = matplotlib.colors.ListedColormap(tier_scale['colors'])
        norm_bar = matplotlib.colors.BoundaryNorm(tier_scale['bounds'], cmap_bar.N)
    ...
    if self.surface3d:
      ...
      if use_burn_tier:
        tier_scale = self._burnTierScale(zdata)
        cmap3d = matplotlib.colors.ListedColormap(tier_scale['colors'])
        norm3d = matplotlib.colors.BoundaryNorm(tier_scale['bounds'], cmap3d.N)
        ...
        if not self.categoricalLegend:
          cbar = fig3d.colorbar(matplotlib.cm.ScalarMappable(norm=norm3d, cmap=cmap3d),
                                ax=ax3d, fraction=0.046, pad=0.08, ticks=tier_scale['ticks'], boundaries=tier_scale['bounds'])
