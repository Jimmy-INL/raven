...
\newcommand{\functionDescription}
{
  ...
  \ref{sec:functions}. This function module must contain and implement a method either
   with the same name of the function or a method named  ``evaluate''.
 \nb{Alternatively, this node must be omitted
...
In order to work properly, this function must contain either a method named with the same name of the function or a method named ``evaluate''. 
Such method needs to return a single python object (float, integer, string). In this way, multiple variables can be associated with the same function.  For example,
\begin{lstlisting}[style=XML]
...
<Functions>
...
\begin{lstlisting}
def torus_calcs(self):
  return self.outer_radius - self.inner_radius
\end{lstlisting}
or the default method named ``evaluate'':
\begin{lstlisting}
def evaluate(self):
  return self.outer_radius - self.inner_radius
\end{lstlisting}
...
printing and DataObjects and Database storage.
...
If the \xmlNode{function}  name is used to define the method, it is possible to implement different functions
in the same python module. For example, if 3 functions need to be implemented, 3 files are required if
the methods are named ``evaluate'' or just 1 file is required if the methods are called with the function name.
For example, we can create 3 functions (IDs) pointing to the same Python file for 3 different variables:
\begin{lstlisting}[style=XML]
...
<Functions>
  <External name='torus_calcs1' file='torus_calcs.py'>
    <variable>outer_radius</variable>
    <variable>inner_radius</variable>
  </External>
   <External name='torus_calcs2' file='torus_calcs.py'>
    <variable>outer_radius</variable>
    <variable>inner_radius</variable>
  </External>
   <External name='torus_calcs3' file='torus_calcs.py'>
    <variable>outer_radius</variable>
    <variable>inner_radius</variable>
  </External>
<Functions>
...
<Samplers>
  <WhatEverSampler name='myExampleSampler'>
    ...
    <variable name='thickness1'>
      <function>torus_calcs1</function>
    </variable>
        <variable name='thickness2'>
      <function>torus_calcs2</function>
    </variable>
        <variable name='thickness3'>
      <function>torus_calcs3</function>
    </variable>
  </WhatEverSampler>
</Samplers>
\end{lstlisting}
The corresponding function file \xmlString{torus\_calcs.py} will contain 3 methods: 
\begin{lstlisting}
def torus_calcs1(self):
  return self.outer_radius - self.inner_radius
def torus_calcs2(self):
  return (self.outer_radius - self.inner_radius)*1.01
def torus_calcs3(self):
  return (self.outer_radius - self.inner_radius)*1.02
\end{lstlisting}
...
\xmlNode{variablesTransformation} is also required for \textbf{MonteCarlo} sampling. This node is used to transform the variables
specified by \xmlNode{latentVariables} in the transformed space of input into variables specified by \xmlNode{manifestVariables}
...
required for \textbf{Grid} sampling. This node is used to transform the variables specified by \xmlNode{latentVariables} in the
transformed space of input into variables specified by \xmlNode{manifestVariables} in the input space. The variables listed
...
\xmlNode{variablesTransformation} is also required for \textbf{SparseGridCollocation} sampler. This node is used to transform
the variables specified by \xmlNode{latentVariables} in the transformed space of input into variables specified by
...
interactions.  At its lowest level (order 1), it treats the function as a sum of the reference case plus a functional of each input dimension separately.  At
...
provide additional information, i.e. the \xmlNode{transformation} under \xmlNode{MultivariateNormal} of \xmlNode{Distributions}
(more information can be found in Section \ref{subsec:NdDist}). In addition, the node \xmlNode{variablesTransformation} is also
required for \textbf{Stratified} sampler. This node is used to transform the variables specified by \xmlNode{latentVariables} in the
transformed space of input into variables specified by \xmlNode{manifestVariables} in the input space. The variables listed
in \xmlNode{latentVariables} should be predefined in \xmlNode{variable}, and the variables listed in \xmlNode{manifestVariables}
 ...
 \item \textbf{Central Composite}: This design consists of three distinct sets
  ...
  \begin{itemize}
    ...
    \item A set of center points, experimental runs whose values of each factor
      ...
      experiment;
    \item A set of axial points, experimental runs identical to the center
...
(more information can be found in Section \ref{subsec:NdDist}). In addition, the node \xmlNode{variablesTransformation} is also
required for \textbf{ResponseSurfaceDesign} sampling. This node is used to transform the variables specified by \xmlNode{latentVariables} in the
transformed space of input into variables specified by \xmlNode{manifestVariables} in the input space. The variables listed
in \xmlNode{latentVariables} should be predefined in \xmlNode{variable}, and the variables listed in \xmlNode{manifestVariables}
are used by the \xmlNode{Models}.
...
%%% Forward Samplers: Saltelli
\subsubsection{Saltelli sampling strategy}
\label{subsubsubsec:Saltelli}
The \textbf{Saltelli} sampler generates the cross-sample design required to
estimate variance-based (Sobol') global sensitivity indices. From a
low-discrepancy Sobol' sequence in $2D$ dimensions (where $D$ is the number of
sampled variables) it forms two independent base matrices $A$ and $B$ of size
$N \times D$, and then $D$ hybrid matrices $AB_i$, where $AB_i$ is a copy of $A$
with its $i$-th column replaced by the $i$-th column of $B$. The unit-hypercube
rows are mapped to the input space through the inverse CDF (\texttt{ppf}) of each
variable's \xmlNode{distribution}, exactly as the other forward samplers do, so
any RAVEN univariate distribution may be used. The total number of model
evaluations is $N(D+2)$; when \xmlNode{computeSecondOrder} is \xmlString{True}
the complementary $BA_i$ matrices are also emitted and the count becomes
$N(2D+2)$. The resulting \textbf{PointSet} is consumed by the
\textbf{SobolIndices} PostProcessor (see Section~\ref{SobolIndices}), which
reconstructs the $A / B / AB_i$ blocks from the sample order and computes the
first-, total-, and (optionally) second-order indices.

The sampler relies only on \texttt{numpy} and \texttt{scipy} (through
\texttt{scipy.stats.qmc.Sobol}); no external sensitivity-analysis library is
required.
\specBlock{a}{Saltelli}
%
\attrsIntro
\vspace{-5mm}
\begin{itemize}
...
In the \textbf{Saltelli} input block the user lists the variables to be sampled
within consecutive \xmlNode{variable} blocks, each associated to a
\xmlNode{distribution}, exactly as for the \textbf{MonteCarlo} sampler. In
addition, the following node configures the design:
\begin{itemize}
  \item \xmlNode{samplerInit}, \xmlDesc{XML node, required parameter}, collects
    the settings of the Saltelli design. It accepts the following sub-nodes:
    \begin{itemize}
      \item \xmlNode{N}, \xmlDesc{integer, required field}, the base sample size
        $N$. The generated number of model evaluations is $N(D+2)$ (or
        $N(2D+2)$ with second-order indices). Must be at least 2.
      \item \xmlNode{initialSeed}, \xmlDesc{integer, optional field}, the seed
        used to initialize the Sobol' sequence engine, for reproducible designs.
      \item \xmlNode{computeSecondOrder}, \xmlDesc{bool, optional field}, when
        \xmlString{True} the $BA_i$ matrices are also emitted so the paired
        \textbf{SobolIndices} PostProcessor can estimate second-order indices.
        \default{False}
      \item \xmlNode{scramble}, \xmlDesc{bool, optional field}, whether to apply
        Owen scrambling to the Sobol' sequence (recommended for better
        convergence of the index estimators). \default{True}
    \end{itemize}
  \item \xmlNode{variable}, \xmlDesc{XML node, required parameter}, as in the
    other forward samplers, associates a sampled variable to a
    \xmlNode{distribution}. \nb correlated variables (a single \xmlNode{variable}
    node listing several comma-separated names bound to one ND distribution) are
    not supported by this sampler.
  \item \constantVariablesDescription
\end{itemize}
...
parameters.
%
Thus, exploring the space of all possible simulation outcomes is unfeasible
...
the case, the user needs to provide additional information, i.e. the \xmlNode{transformation} under \xmlNode{MultivariateNormal}
of \xmlNode{Distributions} (more information can be found in Section \ref{subsec:NdDist}). In addition, the node
\xmlNode{variablesTransformation} is also required for \textbf{AdaptiveSparseGrid} sampler. This node is used to transform
the variables specified by \xmlNode{latentVariables} in the transformed space of input into variables specified by
\xmlNode{manifestVariables} in the input space. The variables listed in \xmlNode{latentVariables} should be predefined
in \xmlNode{variable}, and the variables listed in \xmlNode{manifestVariables}
are used by the \xmlNode{Models}.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    provided or not. When True, the code expects to receive the log likelihood value.
    \default{`False'}
  \end{itemize}
