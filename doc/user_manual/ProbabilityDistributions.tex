...
The support can be defined either by an explicit \xmlNode{values} list, by bounds plus \xmlNode{nPoints},
or by bounds plus \xmlNode{step}. If \xmlNode{nPoints} and \xmlNode{step} are both omitted,
the bounds must be integer-valued and the support is all integers between them (inclusive).

\specBlock{a}{UniformDiscrete}
%
\attrIntro
\vspace{-5mm}
\begin{itemize}
...
\subnodeIntro
\begin{itemize}
  \item \xmlNode{lowerBound}, \xmlDesc{float, conditionally required parameter}, lower bound; required unless \xmlNode{values} is provided.
  \item \xmlNode{upperBound}, \xmlDesc{float, conditionally required parameter}, upper bound; required unless \xmlNode{values} is provided.
  \item \xmlNode{nPoints},    \xmlDesc{integer, optional parameter}, number of points between lower and upper bound (mutually exclusive with \xmlNode{step}).
  \item \xmlNode{step},       \xmlDesc{float, optional parameter}, spacing between adjacent points from lower to upper bound; must evenly divide the range (mutually exclusive with \xmlNode{nPoints}).
  \item \xmlNode{values},     \xmlDesc{comma-separated float list, optional parameter}, explicit support values (mutually exclusive with \xmlNode{lowerBound}, \xmlNode{upperBound}, \xmlNode{nPoints}, and \xmlNode{step}).
...
<Distributions>
  ...
    ...
    <UniformDiscrete name="UD_list_dist">
      <values>2.0, 2.5, 7.25</values>
      <strategy>withReplacement</strategy>
    </UniformDiscrete>
  ...
</Distributions>
