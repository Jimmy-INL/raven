  ...
  \begin{itemize}
    ...
    \item \xmlNode{GAparams}:
      ...
      \begin{itemize}
        ...
        \item \xmlNode{constraintHandling}: \xmlDesc{[penalty, dominance]},
          Defines how constraints influence multi-objective ranking:
          \begin{itemize}                     \item \textit{penalty} - Constraints are folded into
          fitness values (penalty-based).                     \item \textit{dominance} - Uses Deb's
          constraint-domination: feasible solutions dominate infeasible ones,
          and infeasible solutions are ranked by total violation.                   \end{itemize}
        ...
        \item \xmlNode{fitness}: \xmlDesc{string},
          ...
          The \xmlNode{fitness} node recognizes the following parameters:
            \begin{itemize}
              \item \xmlAttr{type}: \xmlDesc{[invLinear, feasibleFirst, logistic], required},
                ...
                fitness values proportional to the objective function values with sign
                determined by min/max, prioritizing better objective values for selection and
                reproduction. It suppoort only single-objective optimization problem.\\\\
                $fitness = s \times a \times obj - b \times \sum_{j=1}^{nConstraint}
                max(0,-penalty_{j})$, where $s=-1$ for minimization and $s=1$ for maximization.\\
                where j represents an index of objects                         \\
                \item \textit{logistic} - It applies a logistic function to transform raw objective
                function values into fitness scores.                         It supports min/max by
                flipping the exponent sign. It suppoort only single-objective optimization
                problem.\\\\                         $fitness = \frac{1}{1+e^{s\times a \times
                (obj-b)}}$, where $s=1$ for minimization and $s=-1$ for maximization.\\
          ...
          \begin{itemize}
            ...
            \item \xmlNode{positiveFitness}: \xmlDesc{[True, Yes, 1, False, No, 0, t, y, 1, f, n, 0]},
              If True, fitness values in the output are shifted so the minimum per objective is non-
              negative.

            \item \xmlNode{positiveFitnessEps}: \xmlDesc{float},
              Minimum fitness value after shifting when positiveFitness is enabled. \default{0.0}
          \end{itemize}
      \end{itemize}
...
\subsection{MultiObjectiveGeneticAlgorithm}
  The \xmlNode{MultiObjectiveGeneticAlgorithm} augments \xmlNode{GeneticAlgorithm} with the
...
\vspace{7pt} \\When used as part of a \xmlNode{MultiRun} step, this entity provides
        ...
        \begin{itemize}
          \item \texttt{trajID}: integer identifier for different optimization starting locations and paths
             ...
             \item \texttt{modelRuns}: integer identifying the number of times the model is evaluated up to the current step
             \item \texttt{conv\_\{CONV\}}: status of each given convergence criteria
             \item \texttt{rank}: Non-dominated sorting rank for each survivor in the population.
             \item \texttt{CD}: Crowding distance used to preserve solution diversity within a front.
             \item \texttt{fitness}: fitness of the current chromosome
             \item \texttt{age}: age of current chromosome
             \item \texttt{batchId}: Id of the batch to whom the chromosome belongs
             \item \texttt{AHDp}: p-Average Hausdorff Distance between populations
  ...
  The \xmlNode{MultiObjectiveGeneticAlgorithm} node recognizes the following parameters:
    \begin{itemize}
      \item \xmlAttr{name}: \xmlDesc{string, required},
        User-defined name to designate this entity in the RAVEN input file.
      \item \xmlAttr{verbosity}: \xmlDesc{[silent, quiet, all, debug], optional},
  ...
  The \xmlNode{MultiObjectiveGeneticAlgorithm} node recognizes the following subnodes:
  \begin{itemize}
    \item \xmlNode{objective}: \xmlDesc{comma-separated strings},
      List the objective variables that jointly define the Pareto front. Two or more
    ...
    \item \xmlNode{convergence}:
      ...
      \begin{itemize}
        ...
        \item \xmlNode{hypervolume}: \xmlDesc{float},
          provides the maximum relative change permitted between consecutive Pareto-front
          dominated hypervolumes before convergence is declared. Smaller values enforce tighter
          convergence of the dominated space.

        \item \xmlNode{spread}: \xmlDesc{float},
          provides the maximum allowable value of Deb's spread metric (Δ) measuring the distance
        ...
        \item \xmlNode{spacing}: \xmlDesc{float},
          provides the maximum allowable spacing metric that captures the variance of
          nearest-neighbour distances among rank-1 solutions. Lower values indicate a uniform
          distribution and trigger convergence.

        \item \xmlNode{maxSpread}: \xmlDesc{float},
          sets the relative-change tolerance for the maximum spread (Euclidean range of objectives)
          observed between successive Pareto fronts. Convergence occurs once the change is below
          this value.

        \item \xmlNode{rank1Ratio}: \xmlDesc{float},
          specifies the minimum proportion of the population that must belong to the first
...
\subsection{NSGAII}
  The \xmlNode{NSGAII} optimizer extends \xmlNode{GeneticAlgorithm} to handle multi-objective
  problems using                             the Non-dominated Sorting Genetic Algorithm II (NSGA-
  II). It preserves all configuration options of the                             base genetic
  algorithm while enabling Pareto front ranking and crowding-distance based survivor selection
...
\vspace{7pt} \\When used as part of a \xmlNode{MultiRun} step, this entity provides
  ...
  The \xmlNode{NSGAII} node recognizes the following parameters:
    \begin{itemize}
      \item \xmlAttr{name}: \xmlDesc{string, required},
        User-defined name to designate this entity in the RAVEN input file.
      \item \xmlAttr{verbosity}: \xmlDesc{[silent, quiet, all, debug], optional},
  ...
  The \xmlNode{NSGAII} node recognizes the following subnodes:
  \begin{itemize}
    \item \xmlNode{objective}: \xmlDesc{comma-separated strings},
      Name of the objective variable(s) to optimize. Provide at least two comma-separated variables
